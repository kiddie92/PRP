...
from datasets.predict_dataset_k400 import PredictDataset
import time
import torch
...
params['dataset'] = '/home/YaoYuan/Dataset/kinetics/'
# params['data']='UCF-101'
params['data'] = 'kinetics-400'
train_epoch = 100
learning_rate = 0.01
...
#         motion_mask = torch.ones_like(motion_mask)
...
#             print(clip_input.size())
#             print(clip_label.size())
#             print(clip_output.size())
...
#             showoneofthem(clip_output[0],root_path,epoch,step,'pd_s{}_r{}'.format(step_label[0].item(),recon_rate[0].item()))
            logger = open(os.path.join(root_path, 'log'), 'a+')
            print('-----------------------------------------------')
            logger.write('\n-----------------------------------------------')
    ...
    logger.write('\n' + str(correct_cls_cnt))
    logger.write('\n' + str(total_cls_cnt))
    logger.write('\n' + str(correct_cls_cnt / total_cls_cnt))
    logger.flush()


def validation(val_loader, model, criterion_MSE, criterion_CE, optimizer, epoch, root_path=None):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses_recon = AverageMeter()
    losses_class = AverageMeter()
...
def parse_args():
    ...
    parser.add_argument('--ma_mode', type=str, default='OneMask', help='motion attention mode')
...
def main():
    ...
    # train
    print(params['data'])
    train_dataset = PredictDataset(params['dataset'], mode="train", dataset=params['data'], args=args);
    if params['data'] == 'kinetics-400':
        val_dataset = PredictDataset(params['dataset'], mode='val', dataset=params['data'], args=args);
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', min_lr=1e-7, patience=40, factor=0.1)
    ...
    for epoch in tqdm(range(start_epoch, start_epoch + train_epoch)):
        ...
        val_loss = validation(val_loader, model, criterion_MSE, criterion_CE, optimizer, epoch, root_path=model_save_dir)
